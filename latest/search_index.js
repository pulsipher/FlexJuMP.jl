...

{
    ...
    "title": "Overview",
    "category": "section",
    "text": "FlexJuMP.jl provides a computational framework to analyze and quantify system flexibility. It was originally developed as a JuMP extension to automate the setup and computation of the flexibility index problem with several different types of uncertainty sets. However, it is currently being expanded to compute a number of useful metrics and carry out helpful analyses. Currently, it capabilities include:Computing the flexibility index with ellipsoidal, hyperbox, or p-norm uncertainty sets\nChecking nominal point (mean) feasibility\nComputing the stochastic flexibility index via Monte Carlo sampling methods\nUsing the flexibility index to expedite the solution to the stochastic flexibility index\nComputing the analytic and feasible centers of the feasible region to better nominal points\nComputing the confidence level which provides a lower bound on the stochastic flexibility index\nRanking the most limiting system constraints via iterative solution of the flexibility index problemThese techniques are described in greater detail in Background.note: Note\nCurrently, FlexJuMP only accepts linear constraints and assumes the random variables to multivariate Gaussian with a certain mean and covariance matrix specified by the user. Development is underway to allow for quadratic constraints and general nonlinear constraints.  "
},
...

{
    ...
    "title": "Background",
    "category": "section",
    "text": "This page provides a background on the theory and methods that form the computational framework that is implemented by FlexJuMP to quantify and analyze system flexibility. This is not a rigorous or complete review and more information is provided here"
},
...

{
    ...
    "title": "User Guide",
    "category": "section",
    "text": "CurrentModule = FlexJuMPThis page provides an overview of how to use FlexJuMP to analyze system flexibility. Detailed explanations on the syntax of each method/function and datatype is provided in Library.The package needs to be loaded along with JuMP.jl in the usual manner:using FlexJuMP, JuMP"
},
...

{
    ...
    "category": "section",
    "text": "The covariance matrix stored in m can be extracted via the getcovariance method.covar = getcovariance(m)The covariance matrix can be set or changed using the setcovariance function which requires the second argument to be covariance::Matrix.setcovariance(m, covar)Note that the specified covariance must be symmetric positive semi-definite, otherwise setcovariance will throw an error. Also, it is important that the covariance matrix appropriately match the number of random variables in m."
},

{
    ...
    "category": "section",
    "text": "Now that the flexibility model m is defined we can solve it (i.e., solve the flexibility index problem). This is done simply by calling the JuMP solve function associated with the model. Thus, for our current example we havesolve(m, active_constr = true)A number of keyword arguments can be passed which are each described in the documentation for solvehook. Here the active_constr::Bool keyword argument is used to turn on the active constraint which enforces the number of active constraints at the solution (this is can be used for systems with linearly independent inequalities). The U::Number keyword argument specifies the slack upper bound which can be changed to improve the solution time for a particular problem. Also, the conic_δ::Bool keyword argument can be used to when an MICP solver is used such as Pajarito.jl."
},
...

{
    ...
    "category": "section",
    "text": "The optimized value of the flexibility index stored in m can now be retrieved by using the getflexibilityindex method.flexibility_index = getflexibilityindex(m)Similarly, the stored flexibility index can be used to obtain the confidence level if an ellipsoidal uncertainty set was used. This can be calculated using the getconfidencelevel function.conf_lvl = getconfidencelevel(m)As discussed in the Uncertainty Set Characterization section, the confidence level provides a lower bound on the stochastic flexibility index.The indexes of the active constraints can be obtained via the getactiveconstraints method.actives = getactiveconstraints(m)If desired, we can also directly extract all of the flexibility data associated with the FlexibilityData type.data = getflexibilitydata(m)"
},
...

{
    ...
    "title": "Examples",
    "category": "section",
    "text": "This page provides two application examples that illustrate how FlexJuMP can be used and highlight some of its capabilities."
},
